...
procedure LireVecteur (var B:Vecteur; n:integer);
{Cette procédure permet de rentrer les résultats des équations rentrées précédemment}
	var i:integer;
begin
	for i:=1 to n do
	begin
		clrscr;
		writeln('Resultat equation ',i,' :');
		read(B[i]);
	end;
end;

procedure EcrireVecteur(X:Vecteur; n:integer);
...
begin
	clrscr;
	writeln('la solution cherchée est:');
	writeln;writeln;
	for i:=1 to n do writeln('X',i,' = ',X[i]:15:15);
end;


procedure FormerMatrice(var A:Matrice; var B:Vecteur; var n:integer);
...
begin
	...
	LireMatrice(A,n);
	writeln('Donnez les composantes de B:');
	LireVecteur(B,n);
end;
...
procedure MethodeDeGauss(var U:Matrice; B2:Vecteur; n:integer);
var X:vecteur; i,k:integer;
...
var A,U:Matrice; B,B2:Vecteur; n:integer;
begin
{Et enfin le programme principal}
{On rentre donc tout d'abors les coefficients et les solutions de l'équation}
	FormerMatrice(A,B,n);
{Ensuite, à partir de l'équation AX=B rentré, on va arriver au système UX=B2}
